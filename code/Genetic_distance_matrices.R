...
# plot
ggplot(dist.sequ.cp4, aes(x = test, y = value)) +
...
sum.dist.sequ.cp3 <- dist.sequ.cp4[test == TRUE, .(
  Mean_dist = mean(value),
  SE = sd(value) / sqrt(.N)
), by = "Genus1"][order(-Mean_dist)]
...
sum.dist.sequ.cp4_all <- dist.sequ.cp4[, .(
  Mean_dist = mean(value),
  SE = sd(value) / sqrt(.N)
), by = "Genus1"][order(-Mean_dist)]
...
ggplot(resizCP, mapping = aes(y = Mean_dist, x = Genus_Size, group = Hybrid_Genus)) +
  ...
  ggrepel::geom_text_repel(mapping = aes(label = ifelse(test = resizCP$Genus %in% c(
    "Carex",
    "Euphrasia",
    "Rosa",
    "Rumex",
    ...
    "Sorbus",
    "Juncus"
  ), yes = as.character(resizCP$Genus), no = NA)), point.padding = 0.5, box.padding = 0.5) +
  theme_bw() +
...
# plot
ggplot(dist.sequ.its4, aes(x = test, y = value)) +
  geom_jitter(col = "tomato", alpha = 0.1) +
  geom_boxplot(alpha = 0) +
  theme_bw(base_line_size = 0) +
...
(bothboxplot <- ggplot(bothbox[Marker %in% c("ITS", "Plastid")], aes(x = test, y = value)) +
  geom_jitter(col = "tomato", alpha = 0.1) +
...
(treeboxplot <- ggplot(bothbox[Marker %in% c("Tree")], aes(x = test, y = log(value))) +
  geom_jitter(col = "tomato", alpha = 0.1) +
  geom_boxplot(alpha = 0, size = 1.5) +
  facet_wrap(~Marker) +
  theme_bw(base_line_size = 0) +
...
#
ggsave(
  filename = "./figures/Main/Genetic_distance_boxplotsd.pdf", plot = bothboxplot,
  device = "pdf", width = 11, height = 11, units = "in"
)
ggsave(
  filename = "./figures/Main/All_distances_boxplots.pdf", plot = save_both_distances,
  device = "pdf", width = 11, height = 11, units = "in"
)

##### Part 2.2: Ranking genera for mean genetic distance #####

sum.dist.sequ.its <- dist.sequ.its4[test == TRUE, .(
  Mean_dist = mean(value),
  SE = sd(value) / sqrt(.N),
  N = .N
), by = "Genus1"][order(-Mean_dist)]
...
# for all genera in the British Flora
sum.dist.sequ.its4_all <- dist.sequ.its4[, .(
  Mean_dist = mean(value),
  SE = sd(value) / sqrt(.N)
), by = "Genus1"][order(-Mean_dist)]
# change the genus name
...
ggplot(resizITS, mapping = aes(y = Mean_dist, x = Genus_Size, group = Hybrid_Genus)) +
  # geom_errorbar(mapping = aes(ymax = Mean_co_dist +SE, ymin = Mean_co_dist-SE), width = 0.5)+
  geom_point(aes(colour = Hybrid_Genus), size = 2) +
  ggrepel::geom_text_repel(mapping = aes(label = ifelse(test = resizITS$Genus %in% c(
    "Carex",
    "Euphrasia",
    "Rosa",
    "Rumex",
    ...
    "Sorbus",
    "Juncus"
  ), yes = as.character(resizITS$Genus), no = NA)), box.padding = 0.5, point.padding = 0.5) +
  theme_bw() +
  xlab(label = "Genus size") +
  ylab(label = "Mean Genetic Distance (ITS)")
...
ggplot(cor.dist[!is.na(value) & !is.na(i.value), ], mapping = aes(x = value, y = i.value, colour = test)) +
  ...
  guides(
    colour = guide_legend(title = "Hybrid formed?"),
