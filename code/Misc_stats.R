...
# only hybridising taxa
data2.4[Genus.Size >=5 & test == TRUE, .(Mdist = mean(value)), by = "Genus1"][order(-Mdist)]
...
data2.5[test == TRUE & Genus.Size >= 5, .(Mdist = mean(value), SE = sd(value)/sqrt(.N)), by = .(Genus1)][order(-Mdist)]
