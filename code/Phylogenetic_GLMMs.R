...
# genus columns
vcv2d[, c("Genus1", "Genus2") := list(gsub(" .*", "", Sp1),
                                     gsub(" .*", "", Sp2))]
...
vcv4d$dist2<-mapply(as.character(vcv4d$Sp1), as.character(vcv4d$Sp2), FUN=function(x,y){Dd[x,y]})
...
write.csv(x = data.table(covar = rownames(summary(mcmc.vcv5d)$solutions)[c(2,3,6)],
...
write.csv(x = specify_decimal(summary(mcmc.vcv5d)$solutions, 4),
          file = "./data/Model_outputs/mcmc.vcv5d.summary.csv")

write.csv(x = cbind(c("Chi2", "df", "p"), specify_decimal(as.data.table(VCVglmm::Wald.test.auto(mcmc.vcv5d)), 4)), 
...
a_a <- pnorm(summary(mcmc.vcv5d)$solutions[1,1] + # intercept
                     ...
                     sd = sqrt(mean(mcmc.vcv5d$VCV[,"units"] + 2*mcmc.vcv5d$VCV[,"T1+T2"] + treevar.vcv5d))) # accounting for phylogeny
...
p1 <- posterior.mode(treevar.vcv5d/rowSums(cbind(treevar.vcv5d, 
                                     2*mcmc.vcv5d$VCV[,"T1+T2"],
                                     mcmc.vcv5d$VCV[,"units"])))
hpd1 <- HPDinterval(treevar.vcv5d/rowSums(cbind(treevar.vcv5d, 
                                  2*mcmc.vcv5d$VCV[,"T1+T2"],
                                  mcmc.vcv5d$VCV[,"units"])))
# species variance
p2 <- posterior.mode(2*mcmc.vcv5d$VCV[,"T1+T2"]/rowSums(cbind(treevar.vcv5d,
                                                     2*mcmc.vcv5d$VCV[,"T1+T2"],
                                                     mcmc.vcv5d$VCV[,"units"])))
hpd2 <- HPDinterval(2*mcmc.vcv5d$VCV[,"T1+T2"]/rowSums(cbind(treevar.vcv5d,
                                                  2*mcmc.vcv5d$VCV[,"T1+T2"],
                                                  mcmc.vcv5d$VCV[,"units"])))
...
write.csv(x = cbind(c("Chi2", "df", "p"), specify_decimal(as.data.table(VCVglmm::Wald.test.auto(mcmc.ploidd)), 4)), 
...
same_ploidy <- pnorm(summary(mcmc.ploidd)$solutions[1,1] + # intercept
                           ...
                           sd = sqrt(mean(mcmc.ploidd$VCV[,"units"] + 2*mcmc.ploidd$VCV[,"T1+T2"] + treevar.ploidd))) # accounting for phylogeny
...
mcmc.ploidvis <- vcv6d[, .(Prob = mean(y),
...
ploid_plot <- function(){
  ...
  y <- pnorm(summary(mcmc.ploidd)$solutions[1,1] + # intercept
             ...
             sd = sqrt(mean(mcmc.ploidd$VCV[,"units"] + 
                              ...
                              Bbar(tree.ploidd, "Species")*mcmc.ploidd$VCV[,"Sp1+Sp2"]))) # homoploid
  ...
  y2<- pnorm(summary(mcmc.ploidd)$solutions[1,1] + # intercept
               ...
               summary(mcmc.ploidd)$solutions[5,1] + # annual perennial
               summary(mcmc.ploidd)$solutions[4,1]*x, # for a range of hectads
             sd = sqrt(mean(mcmc.ploidd$VCV[,"units"] + 2*mcmc.ploidd$VCV[,"T1+T2"] + Bbar(tree.ploidd, "Species")*mcmc.ploidd$VCV[,"Sp1+Sp2"]))) # heteroploid
...
hpd3 <- HPDinterval(treevar.ploidd/rowSums(cbind(treevar.ploidd, 
                                           2*mcmc.ploidd$VCV[,"T1+T2"],
                                           mcmc.ploidd$VCV[,"units"])))
# species variance ~35%
p4 <- posterior.mode(2*mcmc.ploidd$VCV[,"T1+T2"]/rowSums(cbind(treevar.ploidd,
                                                   2*mcmc.ploidd$VCV[,"T1+T2"],
                                                   mcmc.ploidd$VCV[,"units"])))
hpd4 <- HPDinterval(mcmc.ploidd$VCV[,"T1+T2"]/rowSums(cbind(treevar.ploidd,
