...
# save Ainv1 & vcv4
#save(... = Ainv1, file = "./hybridpropensity/data/mcmch1/Ainv1.RData")
#save(... = vcv4, file = "./hybridpropensity/data/mcmch1/vcv4.RData")
#load(file = "./hybridpropensity/data/mcmch1/vcv4.RData")
...
# save this:
# save(updat_sp, file = "./hybridpropensity/data/updat_sp.RData")
# updat_sp <- load(file = "./hybridpropensity/data/updat_sp.RData")
...
# 13.1.20 save vcv5
# fwrite(x = vcv5, file = "./hybridpropensity/data/vcv5.csv")
...
#save(... = vcv5, file = "./hybridpropensity/data/mcmch1/vcv5.RData")
...
#save(tree.vcv5, file = "./hybridpropensity/data/tree.vcv5.RData")
tree.vcv5 # 1110 species
# pull out the inverse matrix
Ainv.vcv5<-inverseA(tree.vcv5, scale=FALSE)$Ainv
#save(... = Ainv.vcv5, file = "./hybridpropensity/data/mcmch1/Ainv.vcv5.RData")

# basic stats
# of all pairwise combinations, how many produce hybrids? 7.7%
vcv5[y == 1, .(N=.N)]/vcv5[,.(N=.N)]
...
#save(... = mcmc.vcv5, file = "./hybridpropensity/data/mcmc.vcv5.RData") ## keep this file local!
#load("./hybridpropensity/data/mcmc.vcv5.RData")
...
hpd1 <- HPDinterval(treevar.vcv5/rowSums(cbind(treevar.vcv5, 
                                  ...
                                  mcmc.vcv5$VCV[,"units"])))
# species variance
p2 <- posterior.mode(2*mcmc.vcv5$VCV[,"T1+T2"]/rowSums(cbind(treevar,
                                                     2*mcmc.vcv5$VCV[,"T1+T2"],
                                                     mcmc.vcv5$VCV[,"units"])))
...
mcmc.vcv5.plot <- function(){
  ...
  ggplot(data.table(x=x, y=y, yup=yup), aes(x = x)) + 
    ...
    geom_vline(xintercept = vcv5[, .(mean(dist2)), by = .(Genus1)][, .(mean(V1))][1]$V1, lty =2, col = "red", size = 2) +
...
mcmc.vcv5.plot()
ggsave(filename = "../figures/Supplementary/Distance_hectads_branch_model1.jpeg", plot = mcmc.vcv5.plot(), width = 12, height = 9, device = "jpeg")
...
ggsave(filename = "../figures/Supplementary/Pairwise_overlap_model1.jpeg", plot = mcmc.vcv5.plot2(), width = 12, height = 9, device = "jpeg")
...
tree_modes3 <- tree_modes2[Variable %in% c(1051, 70, 234, 448)]

jpeg(filename = "../figures/Supplementary/four_phyogenies.jpeg", quality = 100, width = 16, height = 16, units = "in", res = 440)
pdf(file = "../figures/Supplementary/four_phyogenies.pdf", width = 16, height = 16)
# four 'independent' clades. Many nodes will be subtrees of other trees.
par(mfrow = c(2,2), mai = c(0.05, 0.05, 0.05, 0.05))
ape::plot.phylo(treeio::tree_subset(node = 1051, tree = tree.vcv5), font = 4, edge.width = 3)
#text(x = 0.02, y = 13, "Dactylorhiza", font = 2)
ape::plot.phylo(treeio::tree_subset(node = 70, tree = tree.vcv5), font = 4, edge.width = 3)
#text(x = 0.28, y = 30, "Malvids")
ape::plot.phylo(treeio::tree_subset(node = 448, tree = tree.vcv5), font = 4, edge.width = 3)
#text(x = 0.2, y = 40, "Apiaceae")
ape::plot.phylo(treeio::tree_subset(node = 234, tree = tree.vcv5), font = 4, edge.width = 3)
#text(x = 0.03, y = 45, "Fabaceae, Vicia")
dev.off()
...
#save(vcv6, file = "./hybridpropensity/data/vcv6.RData")
### test run on ploidy dataset... ###
...
tree.ploid
#save(tree.ploid, file = "./hybridpropensity/data/tree.ploid.RData")

#load(file = "./hybridpropensity/data/tree.ploid.RData")
# pull out the inverse matrix
...
mcmc.ploid <- MCMCglmm(y ~ dist2 + Cross_Ploid + Hectads_shared + Annual_Perennial + Genus.Size, 
                    ...
                    burnin = 3000*100)
summary(mcmc.ploid)
#save(mcmc.ploid, file = "./hybridpropensity/data/mcmc.ploid.RData")
#load(file = "./hybridpropensity/data/mcmc.ploid.RData")
...
treevar.ploid <- Bbar(phylo = tree.ploid, type = "Species")*mcmc.ploid$VCV[,"Sp1+Sp2"]
...
ploid_plot <- function(){
  ...
  ggplot(data, aes(x = x, y = value, colour = `Parental Ploidy`)) + 
    ...
    geom_vline(xintercept = mean(vcv6$Hectads_shared), lty =2, col = "red", size = 2) +
    xlab(label = expression(paste("Pairwise overlap in distribution", " km"^2))) +
    ylab(label = "Probability of forming a hybrid")+
    theme_bw()+
    theme(axis.title.x = element_text(size = 30),
...
ploid_plot()
ggsave(filename = "../figures/Supplementary/Cross_ploid_hectads.jpeg", plot = ploid_plot(), width = 12, height = 9, device = "jpeg")
ggsave(filename = "../figures/Supplementary/Cross_ploid_hectads.pdf", plot = ploid_plot(), width = 12, height = 9, device = "pdf")

# no confidence intervals on this one.
ploid_plot_2 <- function(){
  ...
  ggplot(data, aes(x = x, y = value, colour = `Parental Ploidy`)) + 
    ...
    geom_ribbon(aes(ymin = lCI, ymax = uCI, fill = `Parental Ploidy`), alpha = 0.15) +
    geom_line(aes(y = uCI), lty = 2, size =1.2) +
    geom_vline(xintercept = mean(vcv6$dist2), lty =2, col = "red", size = 2) +
...
ggsave(filename = "../figures/Supplementary/Cross_ploid_branch.jpeg", plot = ploid_plot_2(), width = 12, height = 9, device = "jpeg")
